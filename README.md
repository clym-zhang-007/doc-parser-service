...
| 解析结果 `meta.blocks_schema_version` | `1.5` | `app/services/document_parse.py` → `BLOCKS_SCHEMA_VERSION` |
...
- **`blocks_schema_version`**：当前为 `1.5`（与 `app/services/document_parse.py` 中常量一致）；分块字段含义或规则变更时递增，便于前后端与离线分析对齐。
